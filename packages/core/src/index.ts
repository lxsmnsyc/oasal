...
function transformServerComponent(
  ...
  programPath.traverse({
    ImportDeclaration(path) {
      // Check if import declaration is a client component
      // /\.client(\.[mc]?[tj]sx?)?$/
      if (options.filter.test(path.node.source.value)) {
    ...
    JSXElement(path) {
      ...
      if (t.isJSXIdentifier(opening.name)) {
        ...
        // Check if the same identifier is marked as a client component
        if (
          binding
          ...
          // Transform all attributes into properties for prop serialization
          // that which the client is going to hydrate with
          for (let i = 0, len = opening.attributes.length; i < len; i += 1) {
            ...
            if (t.isJSXAttribute(attr)) {
              ...
              if (t.isJSXNamespacedName(attr.name)) {
                // Check if the namespace refers
                // to the hydration strategy
          ...
          // Transform the component into a client root
          const ClientID = getHookIdentifier(hooks, path, 'Client', `${options.integration}/server`);
          path.replaceWith(
            t.jsxElement(
              t.jsxOpeningElement(
...
export default function oasal(): babel.PluginObj<State> {
  return {
    name: 'oasal',
    visitor: {
      Program(programPath, state) {
        if (state.opts.source) {
          ...
          transformServerComponent(programPath, state.opts);
        }
      },
    },
  };
